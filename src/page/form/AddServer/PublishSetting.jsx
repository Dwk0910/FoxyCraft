...
export default function PublishSetting() {
    ...
    if (server.servericon.length !== 0 && icon === null) {
        const func = async () => {
            await resizeImageToBase64(server.servericon[0].originFileObj, 64, 64, 'PNG', 100, async (uri) =>
                setIcon(uri),
            );
        };
        void func();
    ...
    for (const item of motdList_1) {
        if (item === '\n') preview_1.push(<span key={i} className={'w-full'}></span>);
            ...
            preview_1.push(
                <span key={i} className={'text-[1.4rem'}>
                    &nbsp;
                </span>,
            );
        else if (isSupported(item))
            preview_1.push(
                <span key={i} className={'font-[MinecraftRegular] text-[#AAAAAA] text-[1.4rem] h-5'}>
                    {item}
                </span>,
            );
        else
            preview_1.push(
                <span key={i} className={'font-[unifont] text-[#AAAAAA] text-[1.1rem] h-5'}>
                    {item}
                </span>,
            );
        i++;
    ...
    return (
        <Form title={'공개 설정'}>
            <div className={'flex flex-col'}>
                ...
                <div
                    className={
                        'flex flex-row justify-start items-center w-full h-25 border-gray-600 border-2 rounded-[5px]'
                    }
                    style={{ backgroundImage: `url(${previewBg})` }}>
                    {icon ? (
                        <img src={icon} className={'h-20 ml-2.5'} alt={'servericon'} />
                    ) : (
                        <img src={defaultPackIcon} className={'h-20 ml-2.5'} alt={'servericon'} />
                    )}
                    <div className={'flex flex-col items-start h-[90%] w-127 ml-3'}>
                        ...
                        <span className={'flex flex-row mt-[2px] w-152 flex-wrap leading-[20px] overflow-y-hidden'}>
                            {preview_1}
                        </span>
                        {/*마스킹 이미지*/}
                    ...
                    <div className={'flex flex-row items-start h-[90%]'}>
                        <span className={'text-[1.4rem] font-[MinecraftRegular] text-[#AAAAAA] mt-1'}>
                ...
                <div className={'flex flex-row w-full border-t-1 border-gray-500'}>
                    <div className={'flex flex-col w-127 h-90'}>
                        ...
                        <textarea
                            className={clsx(
                                'mt-2 mr-5 p-2 min-h-17 border-gray-500 border-1 rounded-[5px] resize-none text-nowrap',
                        ...
                        {warn ? (
                            <div className={'flex flex-row mt-2 text-[1.3rem] ml-1 text-yellow-300 items-center'}>
                                <PiWarningFill />
                                <span className={'font-suite text-[0.8rem] ml-3'}>
                                    비 ASCII 문자가 motd에 포함되었습니다. 미리보기와 다르게 표시될 수 있습니다.
                                </span>
                            </div>
                        ) : (
                            ''
                        ...
                        <div className={clsx('flex flex-row w-full mt-4 transition-all duration-300')}>
                            <span className={'pr-3 mb-3 font-suite text-gray-300'}>
                                서버를 공개할 포트 번호를 입력해주세요
                            </span>
                            <Popover
                                title={<span className={'font-SeoulNamsanM text-[1.1rem]'}>포트 번호</span>}
                                content={
                                    <span className={'font-suite'}>
                                        공유기를 사용하는 사용자에 한해, 이 곳에 입력한 포트에 대해{' '}
                                        <span className={'text-orange-300 underline'}>
                                            포트포워딩이 되어있어야 (포트가 열려있어야)
                                        </span>{' '}
                                        외부에서 정상적인 접속이 가능합니다.
                                        <br />각 공유기의 포트포워딩 방법은 공유기 제조사 설명서나 공식 홈페이지를
                                        참조하세요.
                                    </span>
                                }>
                        ...
                        <div className={'w-full pl-0.5'}>
                            <Input
                                ...
                                onChange={(e) => setServer((prev) => ({ ...prev, port: e.target.value }))}
                            />
                        </div>
                        ...
                        <div className={'w-full pl-0.5'}>
                            <Input
                                placeholder={'최대인원'}
                                value={server.max_player}
                                style={{ width: 100, textAlign: 'center' }}
                                onChange={(e) => {
                                    if (e.target.value < 150)
                                        void setServer((prev) => ({ ...prev, max_player: e.target.value }));
                                    else if (e.target.value >= 150)
                                        void setServer((prev) => ({ ...prev, max_player: 150 }));
                        ...
                        <div className={'flex flex-row mt-5'}>
                            ...
                            <Popover
                                title={<span className={'font-SeoulNamsanM text-[1.1rem]'}>online mode</span>}
                                content={
                                    <span className={'font-suite'}>
                                        마인크래프트 복돌 유저를 차단하는 기능입니다.
                                        <br />
                                        online mode를 비활성화하면 각 플레이어의 아바타가 정상적으로 보이지 않을 수
                                        있습니다.
                                    </span>
                                }>
                                <HiQuestionMarkCircle className={'text-gray-300 mt-1'} />
                        ...
                        <div className={'w-full pl-0.5'}>
                            <Switch
                                value={server.online_mode}
                                onClick={(e) => setServer((prev) => ({ ...prev, online_mode: e }))}
                            />
                        </div>
                    </div>
                    ...
                    `}</style>
                        <Dragger
                                ...
                                if (file.type !== 'image/png') {
                                    toast.error(
                                        <span className={'font-suite text-[1rem]'}>
                                            잘못된 형식의 파일입니다.
                                            <br />(<span className={'font-mono'}>PNG</span> 이미지만 선택할 수
                                            있습니다.)
                                        </span>,
                                    );
                                ...
                                if (newList[0]) {
                                    ...
                                    await resizeImageToBase64(
                                        ...
                                        async (uri) => {
                                            ...
                                            formData.append('file', void base64ToBlob(uri, 'image/png'), 'icon.png');
                            ...
                            <div className={'flex flex-col justify-center items-center'}>
                                {icon ? (
                                    <img src={icon} alt={'server_icon'} width={640} height={640} />
                                ) : (
                                    <>
                                        <SlPicture size={50} className={'text-gray-500 mt-4'} />
                                        <span className={'mt-2 font-suite'}>사용자 지정 서버 아이콘</span>
                                        <span className={'text-gray-400 mb-4 font-suite'}>
                                            아이콘 사이즈 : <span className={'font-mono'}>64x64</span>
                                        </span>
                                    </>
                                )}
                            </div>
