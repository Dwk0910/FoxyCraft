        ...
        <Form title={'추가 설정'}>
            <div className={'flex flex-col w-full'}>
                <div>
                    ...
                    <Switch
                        value={server.custom_world_name}
                        onChange={(e) => setServer((prev) => ({ ...prev, custom_world_name: e }))}
                    />
                </div>
                <div className={'flex flex-row items-center mt-3 ml-2'}>
                    ...
                    <span
                        className={
                            'flex flex-row items-center font-suite text-[#FF3030] ml-3 transition-opacity duration-300'
                        }
                        style={{ opacity }}>
                        ...
                        <Popover
                            ...
                            title={<span className={'font-SeoulNamsanM'}>월드 이름 조건</span>}
                            content={
                                <div>
                                    <span className={'font-suite text-[1rem] text-white'}>
                                        월드 이름은 다음의 조건을 충족하여야 합니다:
                                    </span>
                                    <div className={'flex flex-col ml-3'}>
                                        <span className={'font-suite'}>
                                            <span className={'font-bold'}>·</span>&nbsp;&nbsp;&nbsp;비어있지 않을 것
                                        </span>
                                        <span className={'font-suite'}>
                                            <span className={'font-bold'}>·</span>&nbsp;&nbsp;&nbsp;영어, 숫자,
                                            특수문자로만 이루어질 것
                                        </span>
                                        <span className={'font-suite'}>
                                            <span className={'font-bold'}>·</span>&nbsp;&nbsp;&nbsp;특수문자는{' '}
                                            <span className={'text-orange-400'}>- (하이픈)</span>이나{' '}
                                            <span className={'text-orange-400'}>_ (언더바)</span>만 사용할 것
                                        </span>
                                    </div>
                                </div>
                            }>
                ...
                <div className={'flex flex-col mt-2 ml-2'}>
                    <div className={'flex flex-row items-center'}>
                        ...
                        <Input
                            style={{ width: '70px' }}
                            value={server.auto_backup_period}
                            onChange={(e) => {
                                if (!isNaN(e.target.value) && e.target.value.length > 0)
                                    void setServer((prev) => ({
                                        ...prev,
                                        auto_backup_period: parseInt(e.target.value),
                                    }));
                    ...
                    <div className={'flex flex-row items-center mt-2'}>
                        ...
                        <Input
                            style={{ width: '70px' }}
                            value={server.auto_backup_max_count}
                            onChange={(e) => {
                                if (!isNaN(e.target.value) && e.target.value.length > 0)
                                    void setServer((prev) => ({
                                        ...prev,
                                        auto_backup_max_count: parseInt(e.target.value),
                                    }));
                        ...
                        <Popover
                            title={<span className={'font-SeoulNamsanM'}>월드 백업 개수</span>}
                            content={
                                <span className={'font-suite'}>
                                    최대로 몇 개의 월드 백업을 보존할지 설정합니다.
                                    <br />
                                    이곳에서 설정한 개수의 백업개수 이상으로 백업되면 가장 오래된 백업 순으로 자동으로
                                    지워집니다.
                                </span>
                            }>
