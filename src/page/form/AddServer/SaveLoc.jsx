        ...
        <Form title={'저장 위치 설정'}>
            <div className={'flex flex-row w-full'}>
                <Input
                    placeholder={'저장 폴더'}
                    value={server.path}
                    onChange={(e) => setServer((prev) => ({ ...prev, path: e.target.value }))}
                />
                <input
                    type={'button'}
                    className="ml-5 mr-10 border-1 border-gray-400 rounded-[5px] pl-3 pr-3 font-suite cursor-pointer transition-colors duration-150 hover:border-orange-400"
                    onClick={async () => {
