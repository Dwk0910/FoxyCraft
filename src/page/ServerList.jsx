    ...
    for (const key in serverMap) {
        ...
        array.push(
            <div
                key={key}
                className={clsx(
                    'flex flex-row pl-0.5 items-center h-10 hover:bg-[#707070] cursor-pointer transition-colors duration-200',
                    currentServer.id && currentServer.id === key && 'bg-[#5E5E5E]',
                )}
                onClick={() => changeServer(currentServer.id === key ? '' : key)}>
            ...
            return (
                <div className={'flex flex-col h-full p-6'}>
                    ...
                    <div className={'flex flex-row mt-7 mb-5'}>
                        <div
                            className={clsx(
                                serverStatusMap[currentServer.id] === 'offline' &&
                                    'bg-green-600 cursor-pointer hover:bg-green-700 text-white ',
                                'bg-[#474747] text-gray-400 flex justify-start items-center p-2 w-30 font-suite rounded-[5px] transition-all duration-200',
                            )}
                            onClick={() => openServer(currentServer.id)}>
                            <TbTriangleFilled style={{ transform: 'rotate(90deg)', marginLeft: '10px' }} />
                            <span className={'ml-3'}>서버 시작</span>
                        </div>
                        <div
                            className={clsx(
                                serverStatusMap[currentServer.id] !== 'offline' &&
                                    'bg-red-400 cursor-pointer hover:bg-red-500',
                                'text-white bg-[#474747] flex justify-center items-center p-2 w-10 font-suite rounded-[5px] ml-3 transition-all duration-200',
                            )}>
                            <TbSquareFilled />
                        </div>
                        <div
                            className={
                                'bg-purple-400 text-white flex justify-center items-center p-2 w-10 font-suite rounded-[5px] ml-3 cursor-pointer transition-colors duration-200 hover:bg-purple-500'
                            }>
                            <FaFloppyDisk />
                        </div>
                        <div
                            className={
                                'bg-gray-500 text-white flex justify-center items-center p-2 w-10 font-suite rounded-[5px] ml-3 cursor-pointer transition-colors duration-200 hover:bg-gray-600'
                            }>
                            <IoSettings />
                        </div>
                    ...
                    <div className={'flex flex-row ml-0.5 mt-3'}>
                        <span
                            className={clsx(
                                currentServer.menu === 'console' ? 'border-t-1 pt-[7px]' : 'pt-[8px]',
                                'border-cyan-500 hover:border-cyan-200 hover:border-t-1 hover:pt-[7px] transition-colors duration-300 flex justify-center w-12 mx-0.5 font-SeoulNamsanM cursor-pointer',
                            )}
                            onClick={() =>
                                changeComponent('console', <Console serverUUID={currentServer.id} socket={socket} />)
                            }>
                            로그
                        </span>
                        <span
                            className={clsx(
                                currentServer.menu === 'player' ? 'border-t-1 pt-[7px]' : 'pt-[8px]',
                                'border-cyan-500 hover:border-cyan-200 hover:border-t-1 hover:pt-[7px] transition-colors duration-300 flex justify-center w-19 mx-0.5 font-SeoulNamsanM cursor-pointer',
                            )}
                            onClick={() => changeComponent('player', <Player />)}>
                            플레이어
                        </span>
                        <span
                            className={clsx(
                                currentServer.menu === 'plugin' ? 'border-t-1 pt-[7px]' : 'pt-[8px]',
                                'border-cyan-500 hover:border-cyan-200 hover:border-t-1 hover:pt-[7px] transition-colors duration-300 flex justify-center w-19 mx-0.5 font-SeoulNamsanM cursor-pointer',
                            )}
                            onClick={() => changeComponent('plugin', <Plugin />)}>
                            플러그인
                        </span>
                        <span
                            className={clsx(
                                currentServer.menu === 'mods' ? 'border-t-1 pt-[7px]' : 'pt-[8px]',
                                'border-cyan-500 hover:border-cyan-200 hover:border-t-1 hover:pt-[7px] transition-colors duration-300 flex justify-center w-12 mx-0.5 font-SeoulNamsanM cursor-pointer',
                            )}
                            onClick={() => changeComponent('mods', <Mods />)}>
                            모드
                        </span>
                        <span
                            className={clsx(
                                currentServer.menu === 'backup' ? 'border-t-1 pt-[7px]' : 'pt-[8px]',
                                'border-cyan-500 hover:border-cyan-200 hover:border-t-1 hover:pt-[7px] transition-colors duration-300 flex justify-center w-12 mx-0.5 font-SeoulNamsanM cursor-pointer',
                            )}
                            onClick={() => changeComponent('backup', <Backup />)}>
                            백업
                        </span>
                        <span
                            className={clsx(
                                currentServer.menu === 'settings' ? 'border-t-1 pt-[7px]' : 'pt-[8px]',
                                'border-cyan-500 hover:border-cyan-200 hover:border-t-1 hover:pt-[7px] transition-colors duration-300 flex justify-center w-12 mx-0.5 font-SeoulNamsanM cursor-pointer',
                            )}
                            onClick={() => changeComponent('settings', <Settings />)}>
                            설정
                        </span>
                    </div>
                    ...
                    <motion.div
                        ...
                        initial={{ opacity: 0 }}
                        animate={{ opacity: componentOpacity }}
                    ...
                    <div className={'flex flex-col w-full'}>
                        <div className={'w-full flex flex-row justify-center'}>
                            <input
                                type={'text'}
                                className={
                                    'bg-[#404040] grow ml-3 border-[#404040] text-[0.85rem] border-1 transition-all outline-none duration-200 rounded-[5px] p-2 hover:border-orange-400 focus:border-orange-500'
                                }
                                placeholder="서버 검색"
                            />
                            <span
                                className={
                                    'flex justify-center items-center w-10 ml-2 mr-3 cursor-pointer transition-colors duration-200 rounded-[5px] hover:bg-gray-600'
                                }>
                                <IoFilterOutline />
                            </span>
                        </div>
                        <div className={'flex flex-row mt-3 mb-2 items-center w-full justify-start'}>
                            ...
                            <Popover content={<span className={'font-suite'}>새로운 서버 추가</span>}>
                                <FiPlus
                                    className={
                                        'mt-1 mr-3 cursor-pointer hover:text-gray-300 transition-colors duration-200'
                                    }
                            ...
                            <Popover content={<span className={'font-suite'}>폴더 생성</span>}>
                                <TbFolderPlus
                                    className={
                                        'mt-1 mr-6 cursor-pointer hover:text-gray-300 transition-colors duration-200'
                                    }
                                />
                            </Popover>
                        </div>
                        {(!array || array?.length <= 0) && !loading ? (
                            <div className={'text-center mt-30 text-gray-400 font-suite w-63.5'}>서버가 없습니다</div>
                        ) : (
                            array
                        )}
                        {loading ? (
                            <div className="text-center mt-30 text-gray-400 font-suite w-63.5">로딩 중입니다...</div>
                        ) : (
                            ''
                        )}
                    </div>
