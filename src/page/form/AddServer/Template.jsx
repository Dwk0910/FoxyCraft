...
export default function Template() {
    ...
    function versionRange(start, end) {
        return {
            includes(version) {
                return compareVersion(version, start) >= 0 && compareVersion(version, end) <= 0;
            },
        };
    }
    ...
    const customForm = (
        <>
            <div className={'mt-[15px]'}>
                ...
                <Dragger
                    {...draggerProp}
                    fileList={server.custom_runner_file}
                    onChange={(event) => {
                    ...
                    <div className={'flex flex-col justify-center items-center'}>
                        ...
                        <span className={'font-suite text-gray-400'}>
                            사용 가능한 확장자: <span className={'font-mono'}>.jar</span>
                        </span>
                    </div>
                </Dragger>
            </div>
            <Dropdown
                ...
                <div
                    className={clsx(
                        'flex flex-row justify-between items-center w-full p-2 pl-3.5 mt-3 bg-[#404040] border-1 border-gray-500 rounded-[5px] cursor-pointer transition-all ease-in-out duration-200 hover:border-[#FF8904]',
                        server.custom_runner_file[0] && 'mt-10',
                    )}>
                    {server.custom_jre ? (
                        server.custom_jre
                    ) : (
                        <span className={'font-suite text-gray-400'}>구동기 JRE 버전을 선택해 주세요</span>
                    )}
    ...
    return (
        <Form title={'서버 기본 설정'}>
            ...
            <div className={'flex flex-row justify-start w-full'}>
                <div
                    className={
                        'flex flex-col w-full ml-[3px] pl-3 pb-3 pr-3 mr-3.5 mt-4 h-[500px] border-gray-600 border-1 rounded-[5px]'
                    }>
                    ...
                    <div className={'flex flex-row'}>
                        ...
                        <Switch
                            className={'w-[50px] scale-70'}
                            value={server.custom}
                            onChange={(event) => setServer({ ...server, custom: event })}
                        />
                    </div>
                    {server.custom ? (
                        customForm
                        ...
                        <Cascader
                            ...
                            onChange={(value) => setServer({ ...server, runner: value ? value : [] })}
                            placeholder={'서버 구동기를 선택해 주세요.'}
                        />
                    )}
                    {!server.custom && server['runner'] && getJVMVersion(server['runner']) && (
                        <span className={'text-[0.9rem] font-suite text-gray-300 ml-2 mt-3'}>
                            이 구동기로 연 서버는 {getJVMVersion(server['runner'])}을 기반으로 동작할 것입니다.
                        </span>
                    )}
                </div>
                <div className={'flex flex-col w-full mr-3.5 mt-4 pl-3 pb-3 rounded-[5px]'}>
                    ...
                    <div className={'flex flex-col p-2 h-100 overflow-y-auto'}>
                        <div
                            className={
                                'flex flex-col w-[100%] border-gray-600 border-1 p-2 pl-4 pr-4 rounded-[5px] cursor-pointer transition-all duration-150 ease-in-out hover:scale-102'
                            }
                            onClick={() => {
                                void setServer({
                                    ...server,
                                ...
                                toast.success(
                                    <span className={'font-suite'}>
                                        <span className={'font-bold'}>Paper</span> 구동기로 적용되었습니다.
                                    </span>,
                                );
                            }}>
                            ...
                            <div className={'flex flex-row'}>
                                <div className={'flex flex-col min-w-40'}>
                                    <span className={'font-suite text-[0.9rem]'}>
                                        <span className={'text-gray-300'}>마인크래프트</span>{' '}
                                        <span className={'font-bold'}>1.20.6</span>
                                    </span>
                                    <span className={'font-suite text-[0.9rem]'}>
                                        <span className={'text-gray-300'}>구동기</span>{' '}
                                        <span className={'font-bold'}>Paper</span>
                                    </span>
                                    <span className={'font-suite text-[0.9rem]'}>
                                        <span className={'text-gray-300'}>JRE 버전</span>{' '}
                        ...
                        <div
                            className={
                                'flex flex-col w-[100%] border-gray-600 border-1 p-2 pl-4 pr-4 rounded-[5px] cursor-pointer transition-all duration-150 ease-in-out hover:scale-102 mt-5'
                            }
                            onClick={() => {
                                void setServer({
                                    ...server,
                                ...
                                toast.success(
                                    <span className={'font-suite'}>
                                        <span className={'font-bold'}>Mohist</span> 구동기로 적용되었습니다.
                                    </span>,
                                );
                            }}>
                            <span className={'font-SeoulNamsanB text-[1rem] text-gray-200 mt-2'}>
                                Mohist 추천 모드서버
                            </span>
                            <Divider style={{ marginTop: '10px', marginBottom: '15px' }} />
                            <div className={'flex flex-row'}>
                                <div className={'flex flex-col min-w-40'}>
                                    <span className={'font-suite text-[0.9rem]'}>
                                        <span className={'text-gray-300'}>마인크래프트</span>{' '}
                                        <span className={'font-bold'}>1.7.10</span>
                                    </span>
                                    <span className={'font-suite text-[0.9rem]'}>
                                        <span className={'text-gray-300'}>구동기</span>{' '}
                                        <span className={'font-bold'}>Mohist (Forge)</span>
                                    </span>
                                    <span className={'font-suite text-[0.9rem]'}>
                                        <span className={'text-gray-300'}>JRE 버전</span>{' '}
                                ...
                                <div className={'flex grow justify-center items-center'}>
                                    <img
                                        src={'https://avatars.githubusercontent.com/u/54493246?s=200&v=4'}
                                        className={'h-15'}
                                        alt={'mohist'}
                                    />
                                </div>
